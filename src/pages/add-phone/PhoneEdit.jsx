...
const PhoneEdit = () => {
    ...
    const fetchMission = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/contact/get-phone/${id}`
        );
        const { phoneHeading, phone, phoneImage } = res.data.mission;
        setFormData({ phoneHeading, phone, phoneImage });
      } catch (error) {
    ...
    const handleFileChange = (e) => {
      ...
      setFormData((prev) => ({ ...prev, emailImage: file }));
    };
    ...
    const handleSubmit = async (e) => {
      ...
      formDataToSend.append("phoneHeading", formData.phoneHeading);
      formDataToSend.append("phone", formData.phone);
  
      if (formData.phoneImage) {
        formDataToSend.append("phoneImage", formData.phoneImage); // Include the new image
      }
                ...
                <form onSubmit={handleSubmit}>
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      placeholder="Enter phone Heading"
                    />
                  </div>
