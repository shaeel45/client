    ...
    const getTextData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:5000/api/privacy/get-privacy",
          config
        );
        if (!res.data.status === 401 || !res.data) {
          console.error("Error");
        } else {
          setText(res.data.getPrivacy);
        }
      } catch (error) {
                        ...
                        <td className="p-2 w-1/6">
                          <div className="text-center text-gray-500 flex justify-center text-3xl gap-4">
                            <span className="cursor-pointer">
                              <NavLink to={`/privacy-details/${item._id}`}>
                                <CgDetailsMore />
                              </NavLink>
                            </span>
                            <span className="cursor-pointer text-green-800">
                              <NavLink to={`/privacy-edit/${item._id}`}>
                                <FaRegEdit />
                              </NavLink>
                            </span>
                            <span
