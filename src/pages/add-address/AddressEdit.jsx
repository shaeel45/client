...
const AddressEdit = () => {
    ...
    const fetchMission = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/contact/get-address/${id}`
        );
        const { addressHeading, address, addressImage } = res.data.mission;
        setFormData({ addressHeading, address, addressImage });
      } catch (error) {
    ...
    const handleFileChange = (e) => {
      ...
      setFormData((prev) => ({ ...prev, addressImage: file }));
    };
    ...
    const handleSubmit = async (e) => {
      ...
      formDataToSend.append("addressHeading", formData.addressHeading);
      formDataToSend.append("address", formData.address);
  
      if (formData.addressImage) {
        formDataToSend.append("addressImage", formData.addressImage); // Include the new image
      }
