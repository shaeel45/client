...
const CoreValueDetails = () => {
    ...
    const fetchMissionDetails = async () => {
      try {
        ...
        setMissionData(res.data.corevalue); // Ensure this matches your API response structure
      } catch (err) {
        console.error("Error fetching corevalue details:", err);
        setError("Failed to load corevalue details.");
      } finally {
        setLoading(false);
      }
    };
              ...
              <div className="p-3">
                ...
                <div className="overflow-x-auto">
                  <div className="flex justify-center mb-8">
                    <img
                      src={`http://localhost:5000/Mission/${missionData.image}`}
                      alt={missionData.heading}
                      className="w-52 h-52 rounded-full shadow-lg object-cover"
                    />
                  </div>
                  <h1 className="text-4xl pt-5">Heading</h1>
                  <p className="pt-5">{missionData.heading}</p>
...
export default CoreValueDetails
